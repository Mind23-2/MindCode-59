...
prefix = ''
anno_file = "/root/xidian_wks/mtcnn/anno_strore/anno_train.txt"
im_dir = "/opt_data/xidian_wks/mtcnn/WIDER_FACE/WIDER_train/images"
pos_save_dir = "/root/xidian_wks/mtcnn/data_set/train/12/positive"
part_save_dir = "/root/xidian_wks/mtcnn/data_set/train/12/part"
neg_save_dir = '/root/xidian_wks/mtcnn/data_set/train/12/negative'
save_dir = "/root/xidian_wks/mtcnn/data_set/train/12"
...
for annotation in annotations:
    ...
    while neg_num < 50:
        ...
        if np.max(Iou) < 0.3:
            ...
            f2.write("/root/xidian_wks/mtcnn/data_set/train/12/negative/%s.jpg" % n_idx + ' 0\n')
    ...
    for box in boxes:
        ...
        for i in range(5):
            ...
            if max(-size, -x1) > w:
                delta_x = npr.randint(w, max(-size, -x1))
                print("size: ", size, "x1: ", x1, "w: ", w)
            else:
                delta_x = npr.randint(max(-size, -x1), w)
            if max(-size, -y1) > h:
                delta_y = npr.randint(w, max(-size, -y1))
                print("size: ", size, "y1: ", y1, "w: ", h)
            else:
        ...
        for i in range(20):
            ...
            if iou >= 0.65:
                ...
                f1.write("/root/xidian_wks/mtcnn/data_set/train/12/positive/%s.jpg" % p_idx + ' 1 %.2f %.2f %.2f %.2f\n' % (
                offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                p_idx += 1
            elif iou >= 0.4:
                save_file = os.path.join(part_save_dir, "%s.jpg" % d_idx)
                f3.write("/root/xidian_wks/mtcnn/data_set/train/12/part/%s.jpg" % d_idx + ' -1 %.2f %.2f %.2f %.2f\n' % (
                offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                d_idx += 1
        box_idx += 1
